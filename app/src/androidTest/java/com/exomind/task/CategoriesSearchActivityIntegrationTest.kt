...
import com.exomind.task.weather.CityWeatherResultAdapter
import com.exomind.task.helpers.RecyclerChildView
...
internal class CategoriesSearchActivityIntegrationTest : BaseTest() {
    ...
    fun performScrolling() {
        ...
        onView(withId(R.id.rvCities))
            .perform(
                RecyclerViewActions.scrollToPosition<CityWeatherResultAdapter.ResourceViewHolder>(
                    10
                )
            )
        sleep()
    ...
    private fun clickRepoItem(position: Int, isLongClick: Boolean = false) {
        onView(withId(R.id.rvCities)).perform(
            RecyclerViewActions.actionOnItemAtPosition<CityWeatherResultAdapter.ResourceViewHolder>(
                position,
                RecyclerChildView.clickChildViewWithId(R.id.containerWeather, isLongClick)
            )
        )
    }
